...
**Implementation status (15 July 2026):** In progress. `MV1-03` now has an accepted engineering first pass: borders moved out of the fixed-resolution political-fill viewport into the final screen shader, use derivative-based pixel widths, fade province detail by zoom, distinguish sovereign/subject/coast/lake edges, and enforce passive → occupation → war-goal → hover → selection priority. Province-interior SDF contours have been replaced by a two-channel between-texel edge lattice (`R = vertical`, `G = horizontal`), so every shared adjacency is authored once. Exact owner/realm/coast classification occurs before regional/close dilation; the country SDF remains only as a distant strategic minification fallback. Ordinary province boundaries now have an opaque single-stroke core with narrow edge AA; deliberate hollow/double-ring treatment is reserved for war goals. A dense single-province fixture validates occupation, war goal, hover, and selection composition. War goals use a screen-stable gold double border without replacing side/occupation fill. Command routes use a batched, screen-stable outline plus state-specific solid/dashed language, a geometric invalid-target X, and shortest-path splitting at the wrapped world seam. `MV1-04` exposes Normal, Red-green safe, Blue-yellow safe, and High contrast player profiles; pattern strength and reserved semantic accents are adjusted with the palette. It now also generates a deterministic report for all 1,542 starting land-neighbour pairs, ranks normal and simulated colour-vision risks, weights shared-border exposure, and proposes advisory replacement candidates. `MV1-05` has live subject/appanage and temporary-control semantics; appanage/vassal/personal-union patterns supplement colour while the broader uncolonised/indigenous review remains open. `MV1-06` has a live dark-coast treatment. `MV1-07` now has a complete original placeholder marker family, one stable shield-atlas slot for every country, 39 openly licensed historical/near-period source identities with full provenance, 968 transparent research-required slots with no invented heraldry, flag-backed army markers for sourced countries, atlas-backed battle/siege markers, exact shader count badges, zoom culling, deterministic priority, post-drag click selection, war-inspector focus, stable screen-space clustering, and repeated-click member cycling. A deterministic 720-logical-marker fixture verifies clustering, selection, two draw calls, and a `66.67 ms` event-rebuild P95 ceiling. Canonical inland lakes and tiny-island/shoreline topology fixtures are live. Bespoke art approval, expanded historical flag research, authoritative river content, hands-on colour-vision review, remaining non-conflict marker layers, dense multi-route stress coverage, and the complete capture matrix remain before the Readability Gate can close.
